...
const ExperienceModal: React.FC<ExperienceModalProps> = ({ nodeData }) => {
  return (
    <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
      ...
      {nodeData.projects && (
        <div className="mb-8">
          <h3 className="text-2xl font-semibold text-white mb-6 font-pixelify">Notable Projects</h3>
          <div className="grid gap-4 md:grid-cols-2">
            {nodeData.projects.map((project, index) => (
              <div key={index} className="rounded-lg p-4 border border-gray-700">
                ...
                {project.role && (
                  <p className="text-blue-400 text-sm mb-2">Role: {project.role}</p>
                )}
                {project.duration && (
                  <p className="text-gray-400 text-sm mb-3">Duration: {project.duration}</p>
                )}
      ...
      {/* Education (if relevant to experience) */}
